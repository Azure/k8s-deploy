...
const AzureTraceabilityHelper = require("./utilities/../traceability/azure-traceability-helper");
let kubectlPath = "";
...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!utility_1.isValidAction(action)) {
            core.setFailed('Not a valid action. The allowed actions are deploy, promote, reject');
        }
        else {
            const kubectl = new kubectl_object_model_1.Kubectl(kubectlPath, namespace, true);
            ...
            try {
                if (action === 'deploy') {
                    ...
                    yield deployment_helper_1.deploy(kubectl, manifests, strategy, deploymentConfig);
                }
                else if (action === 'promote') {
                    yield promote_1.promote(kubectl, deploymentConfig);
                }
                else if (action === 'reject') {
                    yield reject_1.reject(kubectl);
                }
            }
            catch (error) {
                status = 'failure';
                ...
                yield AzureTraceabilityHelper.addTraceability(kubectl, deploymentConfig, status);
            }
        }
    });
}

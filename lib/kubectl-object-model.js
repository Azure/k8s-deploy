...
class Kubectl {
    ...
    getDeployedObjects() {
        let manifests = manifest_utilities_1.getManifestObjects(this.deployedManifestsPaths);
        let deployedObjects = [];
        let isKubeObjectPresent = {};
        if (manifests &&
            manifests.length > 0) {
    ...
    storeDeployedManifestsPaths(configurationPaths) {

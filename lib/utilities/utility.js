        ...
        for (const image of imageNames) {
            ...
            try {
                if (!fileHelper.doesFileExist('~/.docker/config.json')) {
                    ...
                    let kubectlArgsPassword = `${imagePullSecret} --template='{{.data.password | base64 --decode }}' `;
                    let pwd = kubectl.executeCommand('get secrets', kubectlArgsPassword);
                    let kubectlArgsUsername = `${imagePullSecret} --template='{{.data.username | base64 --decode }}' `;

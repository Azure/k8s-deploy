        ...
        for (const image of imageNames) {
            ...
            try {
                if (!fileHelper.doesFileExist('~/.docker/config.json')) {
                    ...
                    let kubectlArgsPassword = `secrets/${imagePullSecret} --template='{{.data.password | base64 --decode }}' `;
                    let pwd = kubectl.executeCommand('get secrets', kubectlArgsPassword);
                    let kubectlArgsUsername = `secrets/${imagePullSecret} --template='{{.data.username | base64 --decode }}' `;
                    ...
                    if (pwd.stdout && username.stdout) {
                        let loginArgs = [containerRegistryName, '--username', username.stdout, '--password', pwd.stdout];

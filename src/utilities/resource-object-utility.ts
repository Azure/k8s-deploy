...
export function getResources(
  ...
  filePaths.forEach((filePath: string) => {
    ...
    yaml.safeLoadAll(fileContents, function (inputObject) {
      ...
      if (
        filterResourceTypes.filter((type) =>
          isEqual(inputObjectKind, type, StringComparer.OrdinalIgnoreCase)
        ).length > 0
      ) {
        const resource = {
...
function getImagePullSecrets(inputObject: any) {
  ...
  if (
    ...
    try {
      ...
      core.debug(
        `Fetching imagePullSecrets failed due to this error: ${JSON.stringify(
          ex
        )}`
      );
      return null;
...
function setImagePullSecrets(inputObject: any, newImagePullSecrets: any) {
  ...
  if (
    ...
    try {
      ...
      core.debug(
        `Overriding imagePullSecrets failed due to this error: ${JSON.stringify(
          ex
        )}`
      );
      //Do nothing
